def UNETrun(inno, savestate, type1, network, test, encoder, dim):
...
#    originalnormnp=(originalnp-originalnp.min())/(originalnp.max()-originalnp.min())
   ...
   # picoutnp2 = picavnpnorm>0.5
    ...
    picout.save(test+'/results/picout{}{}{}.tiff'.format(type1,network,encoder))
    im2 = picout 
   ...
   # im = Image.open('test2/testmask.png')
    ...
    im = Image.open(test+'/testmask.tiff')
    target = np.array(im)
