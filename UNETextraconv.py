...
def unet(runname,desc, synthprop,wall1prop,wall2prop, test, inno, batch, path, nepochs, network, encoder, pretrain, dim,transparams, transparamsv):
    ...
    import os
    import sys
    from RUNextraconv import UNETrun
    import wandb
    import datetime
    from torch.utils.tensorboard import SummaryWriter
    from customnet2 import customunet
    import numpy as np
    import torch
   ...
   # try:
    ...
    def train():
       ...
       # loss_fn = nn.CrossEntropyLoss()
        ...
        optimizer2 = torch.optim.Adam(net2.parameters(), lr=0.0004, eps=1e-08,  weight_decay=0.00001)
        ...
        for epoch in range(nepochs):  # loop over the dataset multiple times
            ...
            ltrn = running_loss/n
         #   atrn = correct/total 
            ltst = stats(dataloader_validation, net, net2)
       ...
       #     print(ltrn)
        ...
        savestate2 = '/Final_epoch_{}{}{}2.pt'.format(nepochsf, network, encoder)
        print(savestate)
        saveplace = results_path+savestate
        torch.save({"state_dict": net.state_dict(), "stats": statsrec}, saveplace)
        saveplace2 = results_path+savestate2
        torch.save({"state_dict": net2.state_dict(), "stats": statsrec}, saveplace2)

        if test == 1:    
                iou_score2,iou_score2fit, precision,recall = UNETrun(inno, savestate, savestate2, type1, network, path, encoder, dim, "/test2.tiff","/testmask2.tiff", savetag2)
               # iou_scoresynthetic, precision,recall = UNETrun(inno, savestate, "depth", network, path, encoder, dim, "/testsynth.tiff","/testmasksynth.tiff", "default")
                iou_scoresynthetic,iou_scorefitsynthetic, precision, recall = UNETrun(inno, savestate, savestate2, type1, network, path, encoder, dim, "/testsynth.tiff","/testmasksynth.png", savetagsynthetic)

                iou_score,iou_scorefit, precision,recall = UNETrun(inno, savestate, savestate2, type1, network, path, encoder, dim, "/test.tiff","/testmask.tiff", savetag)
